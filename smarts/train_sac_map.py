...
args.logdir = '/home/haochen/TPDM_transformer/models/'+'model_'+scenario_path[0].split('/')[-1]
# define agent specs
...
for i in range(args.n_experiments):
    ...
    test_env = HiWayEnv(scenarios=scenario_path, agent_specs={AGENT_ID: test_agent_spec}, headless=True, seed=0)
    ...
    policy = SAC(state_shape=OBSERVATION_SPACE.shape, action_dim=ACTION_SPACE.high.size, max_action=ACTION_SPACE.high[0],
                    ...
                    update_interval=1,sep_rep_opt=True)

    bptt_hidden = 0 if not BPTT else 256

    trainer = Trainer(policy=policy,env=env,args=args,test_env=None,save_path=f'ANFsac_map_aughier_{future_steps}_{args.scenario}_{i}',
                            ...
                            test_obs_adapter=test_obs_adapter.obs_adapter,neighbor_spec=neighbor_spec
                            ,path_length=planned_path,params=params,neighbors=neighbors,multi_prediction=multi_pred,
                            multi_selection=multi_selection,pred_future_state=representations,skip_timestep=3,sep_train=True,
                            max_train_episode=0)
