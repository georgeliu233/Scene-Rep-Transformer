    ...
    for i, start in enumerate(start_routes):
        for end in end_routes:
            if end.split('-')[0] != start.split('-')[0]:
                flows.append(Flow(route=Route(begin=('edge-'+start, 0, "random"), end=('edge-'+end, 0, "random")), rate=80, actors=actors))

    flows.append(Flow(route=Route(begin=('edge-east-EN', 1, "random"), end=('edge-south-NS', 0, "random")), rate=50, actors=actors))
