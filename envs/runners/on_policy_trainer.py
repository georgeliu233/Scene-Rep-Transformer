...
class OnPolicyTrainer(Trainer):
    def __init__(self,ego_surr=False,surr_vehicles=5,state_input=False,
        lstm=False,n_steps=1,save_name='./ppo', *args, **kwargs):
    ...
    def __call__(self):
        ...
        self.replay_buffer = get_replay_buffer(
            self._policy, self._env,n_step=self.n_steps)
        ...
        n_epoisode = 0
        obs = self._env.reset()
        init_pos = obs[self._env.agent_id].ego_vehicle_state.position[:2]
        if self.state_input:
            if self.ego_surr:
                obs,neighbor_ids = self.ego_surr_adapter(obs[self._env.agent_id])
            else:
                obs = self.observation_adapter(obs[self._env.agent_id])
        else:
            obs = obs[self._env.agent_id].top_down_rgb.data/255
        ...
        self.train_reward_scaler = ZFilter(shape=(),center=False,gamma=None)
        ...
        while total_steps < self._max_steps:
            ...
            for _ in range(self._policy.horizon):
                ...
                self._env.agent_id: action
                })
                r = reward[self._env.agent_id]
                episode_return += r

                if self.state_input:
                    if self.ego_surr:
                        next_obs,neighbor_ids = self.ego_surr_adapter(next_obs[self._env.agent_id])
                    else:
                        next_obs = self.observation_adapter(next_obs[self._env.agent_id])
                else:
                    next_obs = next_obs[self._env.agent_id].top_down_rgb.data/255.0
                ...
                done_flag = done[self._env.agent_id]
                ...
                if done[self._env.agent_id] or episode_steps == self._episode_max_steps:
            
                    success_log.append(1 if info[self._env.agent_id]['env_obs'].events.reached_goal else 0)
                    tf.summary.experimental.set_step(total_steps)
                    ...
                    if self.state_input:
                        ...
                        obs = obs[self._env.agent_id].top_down_rgb.data 
                    if self.lstm:
                        buffer_queue = deque(maxlen=self.n_steps)
                        # for _ in range(self.n_steps):
                        buffer_queue.append(obs)
    ...
    def evaluate_policy(self, total_steps,plot_map_mode=None):
        ...
        for i in trange(self._test_episodes):
            ...
            init_pos = obs[self._test_env.agent_id].ego_vehicle_state.position[:2]
            if self.state_input:
                if self.ego_surr:
                    self.test_neighbors_buffer.clear()
                    obs,neighbor_ids = self.ego_surr_adapter(obs[self._test_env.agent_id],test=True)
                else:
                    obs = self.observation_adapter(obs[self._test_env.agent_id])
            else:
                obs = obs[self._test_env.agent_id].top_down_rgb.data/255.0
            ...
            for _ in range(self._episode_max_steps):
                ...
                if self._normalize_obs:
                    obs = self._obs_normalizer(obs, update=False)
                act, _, _ = self._policy.get_action_and_val(obs, test=True)
               
                next_obs, reward, done, infos = self._test_env.step({
                self._test_env.agent_id: act
                })
         
                done_events = next_obs[self._test_env.agent_id].events
                r = reward[self._test_env.agent_id]

                if self.state_input:
                    if self.ego_surr:
                        next_obs,neighbor_ids = self.ego_surr_adapter(next_obs[self._test_env.agent_id],test=True)
                    else:
                        next_obs = self.observation_adapter(next_obs[self._test_env.agent_id])
                else:
                    next_obs = next_obs[self._test_env.agent_id].top_down_rgb.data/255.0
                ...
                episode_return += r
                obs = next_obs
                if done[self._test_env.agent_id]:
                    ego_data.append(epi_state)
                    obs = self._test_env.reset()
            
                    init_pos = obs[self._test_env.agent_id].ego_vehicle_state.position[:2]
                    obs = obs[self._test_env.agent_id].top_down_rgb.data

                    info = infos[self._test_env.agent_id]
                    ...
                    if info.collisions !=[]:
                        col_time +=1
                    if info.reached_max_episode_steps:
                        stag_time +=1
                    init_dis=0
                    break
