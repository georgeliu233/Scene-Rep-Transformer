...
class Trainer:
    def __init__(
        ...
        self.smarts = None
        if self.use_map and not self.carla:
    ...
    def __call__(self):
        ...
        if self.use_ego:
            ...
            obs,ego,map_s = obs if self.carla else self.neighbor_obs(obs) 
        else:
            ego=None
            obs = obs[self._env.agent_id]
            map_s = None
                ...
                if total_steps < self._policy.n_warmup:
                    ...
                    if self.bptt_hidden>0:
                        ...
                        if self.use_map:
                            ...
                            action = self._policy.get_action(obs,test=False)
                act = action
            pred_ego = None
            ...
            if total_steps % self._policy.update_interval == 0:
                ...
                if self.pred_future_state and self.sep_train:
                    ...
                    if total_steps % 500==0:
                        self.logger.info(f'similarity_loss:{np.mean(simi_loss)}')
