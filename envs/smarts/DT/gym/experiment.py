...
def experiment(
    ...
    def observation_adapter(observation):
        return collector.observation_adapter(observation)
    ...
    # load dataset
    dataset_path = f'/home/haochen/TPDM_transformer/DT/data/data_'+scenario_path[0].split('/')[-1]+'.pkl'
    ...
    for path in trajectories:
        if mode == 'delayed':  # delayed: all rewards moved to end of trajectory
            path['rewards'][-1] = np.array(path['rewards']).sum()
            path['rewards'][:-1] = 0.
    ...
    def get_batch(batch_size=256, max_len=K):
        ...
        for i in range(batch_size):
            ...
            s.append(np.array(traj['states'][si:si + max_len]).reshape(1, -1, state_dim))
            a.append(np.array(traj['actions'][si:si + max_len]).reshape(1, -1, act_dim))
            r.append(np.array(traj['rewards'][si:si + max_len]).reshape(1, -1, 1))
            if 'terminals' in traj:
                d.append(traj['terminals'][si:si + max_len].reshape(1, -1))
            else:
                d.append(np.array(traj['dones'][si:si + max_len]).reshape(1, -1))
            timesteps.append(np.arange(si, si + s[-1].shape[1]).reshape(1, -1))
            timesteps[-1][timesteps[-1] >= max_ep_len] = max_ep_len-1  # padding cutoff
            rtg.append(discount_cumsum(np.array(traj['rewards'][si:]), gamma=1.)[:s[-1].shape[1] + 1].reshape(1, -1, 1))
    ...
    def eval_episodes(target_rew):
        def fn(model):
            ...
            for _ in range(num_eval_episodes):
                with torch.no_grad():
                    if model_type == 'dt':
                        ret, length,su,co = evaluate_episode_rtg(
                            env,
                            state_dim,
                            act_dim,
                            model,
            ...
            return {
                ...
                f'target_{target_rew}_success': np.mean(success),
                f'target_{target_rew}_coll': np.std(coll),
            }
        return fn
